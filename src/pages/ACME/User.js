...
const UserListRegularPage = () => {
    ...
    if (editId) {
      ...
      addUser(
        ...
        (apiRes) => {
          ...
          setAuthToken(token); // Set the authentication token (assuming this function sets the authentication token)
        },
        (apiErr) => {}
      );
    } else {
  ...
  const onEditClick = (id) => {
    ...
    if (selectedUser) {
      setFormData({
        id: id,
        userValidity: formData.userValidity, // Preserving the existing value, if any (This seems unnecessary)
